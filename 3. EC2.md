...
- **Define Max Price**: Set max spot price and obtain instance if current price < max.
- **Hourly Price Fluctuation**: Spot price varies based on demand and capacity.
- **Grace Period**: If current price > max, stop or terminate instance within 2 minutes.
- **Spot Block**: Reserve instance for 1–6 hours without interruptions (though reclaiming may occur).
...
- **Target Capacity**: Fleet meets capacity with cost constraints.
- **Launch Pools**: Define instance types (e.g., m5.large), OS, Availability Zone.
- **Fleet Strategies**:

  - **lowestPrice**: Chooses lowest-cost pool (good for short workloads).
  - **diversified**: Spreads instances across all pools (better availability, long workloads).
  - **capacityOptimized**: Chooses pool with optimal capacity for instance count.
  - **priceCapacityOptimized**: Selects pools with highest capacity, then lowest price (recommended for most workloads).

- **Automation**: Spot Fleets auto-request instances at lowest price.

### Terminating Spot Instances

![](assets/spot-instances.png)

flowchart TD
%% Spot Request Parameters
subgraph Spot_Request
direction TB
A1["Maximum price"]
A2["Desired number of instances"]
A3["Launch specification"]
A4["Request type: one-time | persistent"]
A5["Valid from, Valid until"]
end%% Spot Request Process
...
P2[Request failed]
P3[Launch instances]
P4[Interrupt (one-time)]
P5[Stop (persistent)]
P6[Interrupt (persistent)]
P7[Start (persistent)]
end
